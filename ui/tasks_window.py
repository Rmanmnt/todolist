...
def tasks_window(root, controller, value, call_back=None):
    ...
    task = controller.get_task(value)
    ...
    if task:
        ...
        ttk.Button(button_frame, text="Delete Task", command=del_task).pack(pady=15)

    else:
        ttk.Label(twindow, text="Task not found!", foreground="red", font=label_font_bold, background="#f5f5f5").pack(pady=40)
